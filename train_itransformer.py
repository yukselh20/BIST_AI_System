...
# --- CONFIG ---
SYMBOL = "THYAO"
LOOKBACK = 96   # iTransformer can work with shorter lookbacks too, but let's try 96 (~1.5 hours)
PREDICTION = 24 # Predict next 24 minutes
BATCH_SIZE = 32
EPOCHS = 10
LR = 0.001
...
def train():
    ...
    dataset = iTransformerDataset(df, LOOKBACK, PREDICTION)
    ...
    model = iTransformer(num_variates=5, lookback_len=LOOKBACK, pred_len=PREDICTION)
    model.to(device)
    
    optimizer = Adam(model.parameters(), lr=LR)
    criterion = nn.MSELoss()
    
    print(f"[*] Starting iTransformer Training on {device}...")
    model.train()
    ...
    # Save
    save_path = os.path.join(project_root, "models", "checkpoints", "itransformer_thyao.pth")
